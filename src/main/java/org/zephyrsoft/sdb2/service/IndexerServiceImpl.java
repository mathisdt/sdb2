...
public class IndexerServiceImpl implements IndexerService<Song> {
	...
	public List<Song> search(IndexType indexType, String searchString, FieldName... fieldsToSearchIn) {
		try {
			...
			QueryParser queryParser = new QueryParser(LUCENE_VERSION, FieldName.LYRICS.name(), new StandardAnalyzer(
				LUCENE_VERSION, new CharArraySet(LUCENE_VERSION, 0, true)));
